...
**Client Keypair:** X25519 keypair for the use with NaCl “crypto_box” protocol.

**Client ID:** the public key part of the Client Keypair (32 bytes).

**Session:** a pair of two client IDs that take turns in sending messages to each other.

Both the app and the wallet create their own Client IDs. 
...
App generates the **InitialRequest**. See messages protocol (link).

App creates a universal link to a target wallet:

```
https://<wallet-url>/ton-connect?
                                 v=2&
                                 id=<base64urlsafe(A)>&
...
Wallet generates its own **Client Keypair** (b,B) and stores alongside app’s info and ID.
...
All requests from the app (except the InitialRequest) and all responses from the wallet that go through the networking Bridge are encrypted. 

Given a binary encoding of message **m**, recipient’s Client ID **X** and sender’s private key **y** the message is encrypted as follows:
...
To decrypt the message **M**, the recipient uses its private key **x** and sender’s public key **Y** (aka Client ID):
